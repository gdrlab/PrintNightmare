...
4. Place the faked printer driver ```reverse.dll``` (in the zipped file) to the SMB path
...
```
7. Create a ```uploads``` and ```extracts``` folder to retrieve files from victim machine.
